...
from dataloader import MOSEIRegression, MOSEIRegression_NCE
...
def get_MOSEI_nce_loaders(path, batch_size=128, valid=0.1, num_workers=0, pin_memory=False):
    trainset = MOSEIRegression_NCE(path=path)
    train_sampler, valid_sampler = get_train_valid_sampler(trainset, valid)
    train_loader = DataLoader(trainset, batch_size=batch_size, sampler=train_sampler, collate_fn=trainset.collate_fn, num_workers=num_workers, pin_memory=pin_memory)
    valid_loader = DataLoader(trainset, batch_size=batch_size, sampler=valid_sampler, collate_fn=trainset.collate_fn, num_workers=num_workers, pin_memory=pin_memory)
    testset = MOSEIRegression_NCE(path=path, train=False)
    test_loader = DataLoader(testset, batch_size=batch_size, collate_fn=testset.collate_fn, num_workers=num_workers, pin_memory=pin_memory)
    return train_loader, valid_loader, test_loader
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', type=str, default='logs/moseas_regression', help='Directory for tensorboard logs')
    ...
    train_loader, valid_loader, test_loader = get_MOSEI_loaders('/content/gdrive/MyDrive/multilogue-net/data/moseas_full_regression.pickle', valid=0.0, batch_size=batch_size, num_workers=0)
    ...
    for e in tqdm(range(n_epochs), desc = 'MOSEAS Regression'):
        ...
        writer.add_scalar("Train Loss - MOSEAS Regression", train_loss, e)
        print("Train Loss - MOSEAS Regression: ", train_loss)
        writer.add_scalar("Test Loss - MOSEAS Regression", test_loss, e)
        print("Test Loss - MOSEAS Regression: ", test_loss)
        writer.add_scalar("Train MAE - MOSEAS Regression", train_mae, e)
        print("Train MAE - MOSEAS Regression: ", train_mae)
        writer.add_scalar("Test MAE - MOSEAS Regression", test_mae, e)
        print("Test MAE - MOSEAS Regression: ", test_mae)
        writer.add_scalar("Train Pearson - MOSEAS Regression", train_pear, e)
        print("Train Pearson - MOSEAS Regression: ", train_pear)
        writer.add_scalar("Test Pearson - MOSEAS Regression", test_pear, e)
        ...
        if best_loss == None or best_loss > test_loss:
            ...
            torch.save(model_to_save.state_dict(), f"/content/gdrive/MyDrive/multilogue-net/logs/moseas_full_regression/epoch{e}_testloss_{best_loss}_testmae_{best_mae}_testpear_{best_pear}.pth")
