...
class RegressionModelNCE(nn.Module):       
    def __init__(self, D_m_party_text,D_m_party_audio,D_m_party_video, D_m_context, D_g, D_p, D_e, D_h, D_a=100, dropout_rec=0.5, dropout=0.5):
        ...
        self.smax_fc    = nn.Linear(14*D_h, 1)
    ...
    def forward(self, U_t1, U_t2, U_a, U_v, U_context, qmask, umask, return_embed = False):
        emotions_f_t1,_ = self.multilogue_net_f_t(U_t1, U_context, qmask)
        emotions_f_t2,_ = self.multilogue_net_f_t(U_t2, U_context, qmask)
        emotions_f_a,_ = self.multilogue_net_f_a(U_a, U_context, qmask)
        emotions_f_v,_ = self.multilogue_net_f_v(U_v, U_context, qmask)
        emotions_f_t1 = self.dropout_rec(emotions_f_t1)
        emotions_f_t2 = self.dropout_rec(emotions_f_t2)
        emotions_f_a = self.dropout_rec(emotions_f_a)
        emotions_f_v = self.dropout_rec(emotions_f_v)
        hidden_t1 = (self.linear(emotions_f_t1))
        hidden_t2 = (self.linear(emotions_f_t2))
        hidden_a = (self.linear(emotions_f_a))
        hidden_v = (self.linear(emotions_f_v))
        hidden_t1 = (self.dropout(hidden_t1))
        hidden_t2 = (self.dropout(hidden_t2))
        hidden_a = (self.dropout(hidden_a))
        hidden_v = (self.dropout(hidden_v))
        # hidden_t = random.choice([hidden_t1, hidden_t2]) 
        
        hidden_at1 = BiModalAttention(hidden_t1, hidden_a)
        hidden_at2 = BiModalAttention(hidden_t2, hidden_a)
        hidden_vt1 = BiModalAttention(hidden_t1, hidden_v)
        hidden_vt2 = BiModalAttention(hidden_t2, hidden_v)
        # hidden_tv = BiModalAttention(hidden_t, hidden_v)  
        hidden_va = BiModalAttention(hidden_v, hidden_a)
        
        temp = self.smax_fc(torch.cat([hidden_at1(),hidden_at2(), hidden_vt1(), hidden_vt2(), hidden_va(), hidden_t1, hidden_t2,hidden_a, hidden_v],dim=-1)).squeeze()
        if return_embed:
            return temp
        else:
            pred = 3 * torch.tanh(temp) 

            return pred.transpose(0,1).contiguous().view(-1), hidden_at1(), hidden_at2(), hidden_vt1(), hidden_vt2()

class RegressionModelNCE_(nn.Module):       
    def __init__(self, D_m_party_text,D_m_party_audio,D_m_party_video, D_m_context, D_g, D_p, D_e, D_h, D_a=100, dropout_rec=0.5, dropout=0.5):
        super().__init__()
        self.D_m_party_text = D_m_party_text
        self.D_m_party_audio = D_m_party_audio
    ...
    def forward(self, U_t1, U_t2, U_a, U_v, U_context, qmask, umask, return_embed = False):
        ...
        if return_embed:
            ...
            return {'pred': pred.transpose(0,1).contiguous().view(-1), 
                    't1': hidden_t1,
                    't2': hidden_t1,
                    'a': hidden_a,
                    'v': hidden_v,
...
# reference: https://github.com/antoine77340/MIL-NCE_HowTo100M/blob/master/loss.py
class CrossLingualNCELoss(nn.Module):
    def __init__(self, visual=True, audio=False):
        super(CrossLingualNCELoss, self).__init__()
        self.visual = visual
        self.audio = audio
    def forward(self, hidden_at1, hidden_at2, hidden_vt1, hidden_vt2):
        
        # shape of hidden_at1, hidden_at2, hidden_vt1, hidden_vt2 is (T,B,F) (i hope)
        # want to transform these into shapes (T*B, F)
        re_hidden_at1 = hidden_at1.reshape(-1, 200)
        re_hidden_at2 = hidden_at2.reshape(-1, 200)
        re_hidden_vt1 = hidden_vt1.reshape(-1, 200)
        re_hidden_vt2 = hidden_vt2.reshape(-1, 200)
        del hidden_at1
        del hidden_at2
        del hidden_vt1
        del hidden_vt2
        
        # hidden_at1 and hidden_at2
        if self.audio:
            x1 = torch.matmul(re_hidden_at1, re_hidden_at2.t())
            x1 = x1.view(re_hidden_at1.shape[0], re_hidden_at1.shape[0], -1)
            numerator1 = x1 * torch.eye(x1.shape[0])[:,:,None].cuda()
            numerator1 = torch.logsumexp(numerator1, dim=1)
            denominator1 = torch.cat((x1, x1.permute(1,0,2)), dim=1).view(x1.shape[0], -1)
            denominator1 = torch.logsumexp(denominator1, dim=1)
        
            nceloss1 = torch.mean(denominator1-numerator1)
        else:
            nceloss1 = 0
        
        # hidden_vt1 and hidden_vt2
        if self.visual:
            x2 = torch.matmul(re_hidden_vt1, re_hidden_vt2.t())
            x2 = x2.view(re_hidden_vt1.shape[0], re_hidden_vt1.shape[0], -1)
            numerator2 = x2 * torch.eye(x2.shape[0])[:,:,None].cuda()
            numerator2 = torch.logsumexp(numerator2, dim=1)
            denominator2 = torch.cat((x2, x2.permute(1,0,2)), dim=1).view(x2.shape[0], -1)
            denominator2 = torch.logsumexp(denominator2, dim=1)
            
            nceloss2 = torch.mean(denominator2-numerator2)
        else:
            nceloss2 = 0
...
class InterNCELoss(nn.Module):
    def __init__(self, visual=True, audio=False):
        super(InterNCELoss, self).__init__()
        self.visual = visual
        self.audio = audio
    def forward(self, hidden_t1, hidden_t2):
        re_hidden_t1 = hidden_t1.reshape(-1, 200)
        re_hidden_t2 = hidden_t2.reshape(-1, 200)
        del hidden_t1
        del hidden_t2
        
        
        x1 = torch.matmul(re_hidden_t1, re_hidden_t2.t())
        x1 = x1.view(re_hidden_t1.shape[0], re_hidden_t1.shape[0], -1)
        numerator1 = x1 * torch.eye(x1.shape[0])[:,:,None].cuda()
        numerator1 = torch.logsumexp(numerator1, dim=1)
        denominator1 = torch.cat((x1, x1.permute(1,0,2)), dim=1).view(x1.shape[0], -1)
        denominator1 = torch.logsumexp(denominator1, dim=1)
        
        nceloss = torch.mean(denominator1-numerator1)
